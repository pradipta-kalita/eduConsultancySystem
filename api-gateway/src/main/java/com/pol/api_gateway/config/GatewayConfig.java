    ...
    public RouteLocator myRoutes(RouteLocatorBuilder builder){
        return builder.routes()
                .route("auth_route", r->r.path("/auth/**")
                        ...
                        .uri("lb://USER-SERVICE"))
                .route("profile_route_student",r->r.path("/profile/student")
                        .filters(f->f.filter(((exchange, chain) -> jwtAuthFilter(exchange,chain,STUDENT_ONLY))))
                        .uri("lb://USER-SERVICE")
                )
                .route("profile_route_parent",r->r.path("/profile/parent")
                        .filters(f->f.filter(((exchange, chain) -> jwtAuthFilter(exchange,chain,PARENT_ONLY))))
                        .uri("lb://USER-SERVICE")
                )
                .route("profile_route_admin",r->r.path("/profile/admin")
                        .filters(f->f.filter(((exchange, chain) -> jwtAuthFilter(exchange,chain,ADMIN_ONLY))))
                        .uri("lb://USER-SERVICE")
                )
                .build();
    }
